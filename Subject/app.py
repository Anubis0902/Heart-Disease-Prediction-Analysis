...
with tab1:
    ...
    if submit_btn:
        ...
        try:
            ...
            # In this dataset:  class 0 = HIGH RISK (Heart Disease),  class 1 = LOW RISK (Healthy)
            # probabilities[0] = probability of being HIGH RISK
            # probabilities[1] = probability of being LOW RISK
            prob_high_risk = probabilities[0] * 100   # probability of Heart Disease
            prob_low_risk  = probabilities[1] * 100   # probability of being Healthy
        except AttributeError:
            prob_high_risk = None
            prob_low_risk  = None

        # prediction == 0  =>  HIGH RISK  (Heart Disease present)
        # prediction == 1  =>  LOW RISK   (Healthy)
        if prediction == 0:
            st.error("### ⚠️ High Risk of Heart Disease")
            st.markdown("The model detected clinical indicators strongly correlating with heart disease.")
            if prob_high_risk is not None:
                st.write(f"Estimated Probability of Heart Disease: **{prob_high_risk:.2f}%**")
        else:
            st.success("### ✅ Low Risk of Heart Disease")
            st.markdown("The patient's clinical profile suggests a low probability of heart disease.")
            if prob_low_risk is not None:
                st.write(f"Estimated Probability of Being Healthy: **{prob_low_risk:.2f}%**")
...
with tab2:
    ...
    with col_v1:
        # target=0 → High Risk (red), target=1 → Low Risk (blue)
        fig1 = px.histogram(df, x="age", color="target", barmode="overlay",
                            title="Age Distribution by Target",
                            labels={"target": "Heart Disease (0=High Risk, 1=Low Risk)", "age": "Age in Years"},
        ...
        fig_cp = px.histogram(df, x="cp", color="target", barmode="group",
                            ...
                            labels={"target": "Heart Disease (0=High Risk, 1=Low Risk)", "cp": "Chest Pain Type"},
                            color_discrete_map={0: '#e74c3c', 1: '#3498db'})
    ...
    with col_v2:
        fig2 = px.box(df, x="target", y="thalach", color="target",
                      ...
                      labels={"target": "Heart Disease (0=High Risk, 1=Low Risk)", "thalach": "Max Heart Rate Achieved"},
                      color_discrete_map={0: '#e74c3c', 1: '#3498db'})
        st.plotly_chart(fig2, use_container_width=True)

        fig4 = px.scatter(df, x="age", y="thalach", color="target",
                          title="Age vs Max Heart Rate (Colored by Target)",
                          labels={"target": "Heart Disease (0=High Risk, 1=Low Risk)", "age": "Age", "thalach": "Max Heart Rate"},
                          color_discrete_map={0: '#e74c3c', 1: '#3498db'}, opacity=0.7)
